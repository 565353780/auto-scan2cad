...
def random_jitter(max_jitter: int = 4):
...
def rotation_augmentation_fixed(grid: np.array,
                                num_rotations=None) -> np.array:
...
def rotation_augmentation_interpolation_v2(grid: np.array,
                                           rotation=None) -> np.array:
    if rotation is None:
    ...
    center = np.tile(center.reshape(3, 1),
                     [1, max_size[0] * max_size[1] * max_size[2]])
    grid_coords = np.array(np.unravel_index(
        np.arange(max_size[0] * max_size[1] * max_size[2]),
    ...
    grid_coords = grid_coords[:, 0] * max_size[1] * max_size[
        2] + grid_coords[:, 1] * max_size[2] + grid_coords[:, 2]
    mult = torch.arange(num).view(-1,
                                  1) * max_size[0] * max_size[1] * max_size[2]
    grid_coords = grid_coords + mult
    grid_coords = grid_coords.long()
    scan_rots = scans.permute(0, 2, 3, 4, 1).contiguous().view(-1,
                                                               1)[grid_coords]
    scan_rots = scan_rots.view(scans.shape[0], scans.shape[2], scans.shape[3],
                               scans.shape[4],
                               scans.shape[1]).permute(0, 4, 1, 2, 3)
    scan_rots = scan_rots.numpy()
    return scan_rots[0]


def rotation_augmentation_interpolation(grid: np.array,
                                        rotation=None) -> np.array:
    if rotation is None:
        angle = random_degree()
    else:
        angle = rotation
    grid = scipy.ndimage.rotate(grid,
                                angle, (1, 3),
                                False,
                                prefilter=True,
                                order=3,
...
def jitter_augmentation(grid: np.array, jitter=None) -> np.array:
    ...
    pad = np.pad(grid, ((0, 0), (start[0], end[0]), (start[1], end[1]),
                        (start[2], end[2])),
                 "constant",
                 constant_values=(0, 0))
    ...
    grid = pad[:, offset_start[0]:offset_end[0], offset_start[1]:offset_end[1],
