...
from global_to_patch_retrieval.Data.sample import Sample
...
def load_sample_info(filename: str):
...
def load_raw_df(filename: str) -> Sample:
    ...
    s.tdf = struct.unpack('f' * n_elements,
                          raw[start_index:start_index + n_elements * 4])
    s.tdf = np.asarray(s.tdf,
                       dtype=np.float32).reshape([1, s.dimz, s.dimy, s.dimx])
...
def load_mask(filename: str) -> Sample:
    ...
    # Load binary values
    s.tdf = struct.unpack('B' * n_elements,
                          raw[start_index:start_index + n_elements])
    s.tdf = np.asarray(s.tdf, dtype=np.dtype("?")).reshape(
        [1, s.dimz, s.dimy, s.dimx])
    s.tdf = s.tdf.transpose((0, 3, 2, 1))
